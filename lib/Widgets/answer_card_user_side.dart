...
class AnswersCardUser extends StatefulWidget {
  final String myTitle;

  const AnswersCardUser({Key? key, required this.myTitle}) : super(key: key);

  @override
  _AnswersCardUserState createState() => _AnswersCardUserState();
}

class _AnswersCardUserState extends State<AnswersCardUser> {
  ...
  Future<List<Map<String, dynamic>>> _initialize() async {
    try {
      currentUser = await AuthService(context).getCurrentUser();
      if (currentUser == null) {
