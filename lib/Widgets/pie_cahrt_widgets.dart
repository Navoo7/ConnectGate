...
class PieChartWidget extends StatefulWidget {
  ...
  const PieChartWidget({super.key, required this.optionPercentages});
...
class _PieChartWidgetState extends State<PieChartWidget> {
  ...
  List<PieChartSectionData> _showingSections(List<Color> colors) {
    return widget.optionPercentages.entries.map((entry) {
      ...
      final fontSize = isTouched ? 8.8 : 9.5;
      final radius = isTouched ? 47.0 : 40.0;
      const shadows = [Shadow(color: Colors.black, blurRadius: 2)];

      return PieChartSectionData(
        color: colors[index % colors.length],
        value: entry.value,
        title: isTouched ? entry.key : '${entry.value.toStringAsFixed(1)}%',
        radius: radius,
