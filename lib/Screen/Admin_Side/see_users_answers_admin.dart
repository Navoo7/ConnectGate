...
// // // Replace your existing image display code in the FlexibleSpaceBar background:
  ...
  Future<void> downloadCSV() async {
    ...
    try {
      ...
      // Check for image and download it if available
      String? imageFilePath;
      if (imageUrl != null && imageUrl!.isNotEmpty) {
        imageFilePath = '${directory.path}/${title!}-image.jpg';
        final response = await HttpClient().getUrl(Uri.parse(imageUrl!));
        await response.close().then((fileResponse) async {
          final imageFile = File(imageFilePath!);
          await fileResponse.pipe(imageFile.openWrite());
        });
      }
      ...
      if (imageFilePath != null) {
        zipEncoder.addFile(File(imageFilePath));
      }
      zipEncoder.close();
